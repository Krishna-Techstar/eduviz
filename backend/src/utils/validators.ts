...
function validateTreeNode(x: unknown, depth = 0): x is BinaryTreeNode {
  if (depth > 500) return false;
  if (x === null || x === undefined) return false;
  if (typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  if (!("val" in o)) return false;
  const v = o.val;
  if (typeof v !== "number" && typeof v !== "string") return false;
  if (o.left !== undefined && o.left !== null && !validateTreeNode(o.left, depth + 1)) return false;
  if (o.right !== undefined && o.right !== null && !validateTreeNode(o.right, depth + 1)) return false;
  return true;
}
